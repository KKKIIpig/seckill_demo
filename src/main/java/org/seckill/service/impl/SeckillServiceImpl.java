...
public class SeckillServiceImpl implements SeckillService {
    ...
    public Exposer exportSeckillUrl(long seckillId) {
         ...
         *  locgoin
         */
        // 超时的基础上维护一致性，秒杀商品一般不允许修改，一般是重建
        Seckill seckill = redisDao.getSeckill(seckillId);    //存储秒杀商品的信息
        if (seckill == null){
        ...
        if (nowTime.getTime() < startTime.getTime()   //秒杀还未开始 或 秒杀已经结束
            || nowTime.getTime() > endTime.getTime()){
             return new Exposer(false, seckillId, nowTime.getTime(), startTime.getTime(),endTime.getTime());
        }
        // 任意字符串转化为特定长度字符串的过程，不可逆（用户猜不到MD5是怎么转换的）
    ...
    private String getMD5(long seckillId){
        //如果只对seckill做MD5转换的话，用户是可以通过固定的MD5算法去得到MD5固定值
        //但是加上“/”和盐值时，因为用户不知道我们的MD5的拼接规则和盐值，所以用户是不知道我们的MD5是怎么产生的
    ...
    public SeckillExecution executeSeckill(long seckillId, long userPhone, String md5)
        ...
        if (md5 == null || !md5.equals(getMD5(seckillId))){  //判断用户的MD5和我们生成的MD5是否匹配，
            throw new SeckillException("seckill data rewrite");  //若不匹配，则说明数据被篡改，报异常
        }
        // 减少库存 + 记录购买行为
        // 首先获取当前系统时间
        Date nowTime = new Date();
        try {
            //  记录购买行为  唯一：seckillId, userPhone （索引）
            int insertCount = successKilledMapper.insertSuccessKilled(seckillId, userPhone);
            if (insertCount <= 0){
                // 重复秒杀
                throw new RepeatKillException("seckill repeat");
            } else { //  记录购买行为成功，减少库存
                //传入当前系统时间，与秒杀商品有效时间作比较
